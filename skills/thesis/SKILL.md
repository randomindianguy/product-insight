---
name: thesis
description: Find a claim about a company that survives being killed, and name the one claim it dies without, for a PM job application. Use when the user shares a JD, wants a product insight or application artifact, or asks for a differentiated angle on a company.
---
...
This skill has nine steps in two phases.

**Find (Steps 1–5)** produces a claim that feels strong. **Test (Steps 6–9)** tries to break the claim that feels strong.
...
So there is a hard stop at Step 5. Do not describe an artifact, propose a build, or hand off until Steps 6 through 9 have each run and reported. If you find yourself reaching for "here's what to build" before the two checks have run, stop — that is the skip, and it is the one move this skill exists to prevent.
...
## Say it in plain English — the whole way through

The words this skill uses to work — keystone, wall, receipt, transfer test, producibility flag, segment, the bet — are internal. They are how the skill thinks, not how you talk to the person using it. None of them appears in anything the user reads: not the questions you ask mid-run, not the verdicts you report, not the final summary.

So don't write "the keystone is Assumed." Write "the whole thing rests on X, and I haven't actually checked X." Don't write "that fails the segment-validity check." Write "those two groups would buy the same product, so splitting them tells you nothing." If a sentence can't be understood without the glossary, it isn't finished — rewrite it until a smart person who has never seen this skill gets it on the first read.

The one exception is the final handoff to `/artifact`: that is one machine briefing another, and the shorthand is fine there. Everywhere a human reads, plain English.

## Step 1 — Extract the bar
...
**Done when:** one sentence naming their standard, sourced to something they shipped.
...
Then run the segment-validity check — the test that separates a real segment from a marketing dimension. A dimension is only a segmentation dimension if the drivers (what people need, desire, fear) differ so much across it that the two ends fundamentally require a different product. So: name what you would build for the hurting cell, and what you would build for the opposite cell. If it's the same product with different messaging, the dimension is marketing, not segmentation — the drivers don't diverge enough — and you must re-cut on a dimension where the two ends genuinely need different things built. (Clay passes: the accountable-at-scale cell needs a verification layer built; the unaccountable-low-scale cell needs nothing of the sort — different products, so the cut is real.)

This check also handles the abstraction level for free. If the only "different products" you can name are all in the same category as the one you're analysing, you've described the product back to yourself, not the job — climb one rung and restate the job more abstractly, until genuinely different kinds of solution show up as the alternatives.

**Done when:** one cell, one causal dimension, the job stated in the fixed format, the segment-validity check passed (the two ends need different products built), and one sentence on why this company wants that cell served.
...
This step runs the transfer test on the models the user carries. The test is one thing:
...
Run it now, early, so a bad import gets caught before the thesis is built around it. For each model in the profile that the thesis leans on:
...
## Step 8 — Two checks before it ships

The thesis now exists — claim, wall, segment, bet. Before it leaves this skill, run two checks on the finished thesis. Each can kill or redirect a thesis that passed every earlier step; a check that fails does not just flash red — it routes to the step that owns the failure.

**The counter — does the strongest objection survive?** State the strongest idea against this thesis. Not a weak strawman — the best case a smart skeptic would make. The most dangerous form is a competing wall: the thesis named one reason the company hasn't acted, but there are usually several, and the flattering one (they chose to sit on it) is rarely the real one. If the strongest counter is they are already shipping this or their data cannot support it, that counter is the real wall and the thesis dies on it. → Fails when the strongest opposing idea survives scrutiny. Routes to Step 5 — the wall step took one answer where several fit the evidence, and picked the one that flattered the build.

**Producibility — can you build the evidence honestly?** Trace the idea to its destination. If it worked, what would you hand the reader? Then: can that outcome actually be produced from outside the company? An outcome that needs the company's private data, or access you do not have, hits a wall. A thesis can be correct and still lead somewhere unbuildable. → Fails when the destination requires data or access you cannot get. Routes forward to the `/artifact` build gate as a producibility flag. This does not kill the thesis — it may be right and merely unbuildable now — but the build cannot begin until the load-bearing outcome is shown to be producible honestly. A produced-by-fabrication outcome fails severability on sight.

**Done when:** both checks have a written verdict — PASS with the one-line reason, or FAIL with the route taken. Not "looks fine." Two named verdicts, in this shape:

```
Counter — [strongest objection]: PASS/FAIL — [objection defeated / routed to Step 5 because…]
Producibility — [destination]: PASS/FLAG — [producible / flagged to build gate because…]
```

A verdict of PASS requires stating why it passed, not just asserting it did. "Counter: PASS" with no defeated objection written out is the rubber-stamp, and the rubber-stamp is the skip wearing a checkmark. If you cannot write the strongest objection, you have not run the check.

Only after both verdicts are written, and any FAIL has been routed and resolved, does the thesis leave this skill.

## Step 9 — The keystone

The two checks test the thesis from the outside. This step finds the one brick that, pulled out, collapses the whole thing — and checks whether that brick is actually load-bearing-solid or just assumed.
...
Then check the second kind specifically: look back at the two checks and Step 3. Did either check pass on a specific you never flagged? The segment you narrowed to, the mechanism you named — if it's precise and it went unquestioned, it is a candidate keystone hiding as precision. Rate it too.
...
The plain-English rule above governs the whole run; the final summary is where it matters most, so hold the line hardest here.

So end every run with a short summary, under a heading the user can't miss. No glossary terms — if one feels unavoidable, that means the sentence isn't finished, so rewrite it, don't define it in place. Write it the way you'd explain it to a smart friend who has never heard these words. Keep it short — six lines, one per point:
...
*Machine-to-machine handoff — internal shorthand on purpose, written for `/artifact`, not for the user. The plain-English summary above is the human-facing close of the run; don't let this block be the last thing the user reads.*

Hand off to `/artifact`, carrying the two check verdicts, any producibility flag, and the keystone with its rating. The thesis is not the deliverable and does not get written up here.

The check verdicts and the keystone rating travel with the thesis. `/artifact` will not begin a build for a thesis that arrives without them, because a thesis that skipped the checks or never named its keystone is a claim that only feels strong, and building on it is the skip completing itself. If the keystone is Assumed, verifying it is the first task in `/artifact` — before any build, confirm the one claim the thesis dies without.
